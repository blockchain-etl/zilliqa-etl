...
Airflow DAGs for exporting and loading the Zilliqa blockchain data to Google BigQuery:
...
2. Create a Google Cloud Composer environment:
    ...
    gcloud composer environments create ${ENVIRONMENT_NAME} --location=us-central1 --zone=us-central1-a \
        --disk-size=30GB --machine-type=n1-standard-1 --node-count=3 --python-version=3 --image-version=composer-1.11.3-airflow-1.10.9 \
        --network=default --subnetwork=default
    
    gcloud composer environments update $ENVIRONMENT_NAME --location=us-central1 --update-pypi-package=zilliqa-etl==0.0.2
    ```
...
| `provider_uris` | comma-separated list of provider URIs for [zilliqa-etl](https://zilliqa-etl.readthedocs.io/en/latest/commands) command |
...
| `export_max_workers` | max workers for [zilliqa-etl](https://zilliqa-etl.readthedocs.io/en/latest/commands) command, default: `5` |
...
- Specify the following configuration options for the trigger:
    ...
    - Substitution variables:
        - `_ENVIRONMENT_NAME`: `Cloud Composer environment name`, e.g. `_ENVIRONMENT_NAME`: `zilliqa-etl-0`
...
[GKE console](https://console.cloud.google.com/kubernetes/workload?project=zilliqa-etl). 
